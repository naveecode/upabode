...
# Replace the broken VideoPlayer
video_player_regex = re.compile(r'function VideoPlayer.*?return \(.*?</div>\s*\);\s*\}', re.DOTALL)

new_video_player = '''function VideoPlayer({ src, isActive }: { src: string, isActive: boolean }) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [isMuted, setIsMuted] = useState(false); // Autoplay usually requires muted on first load on mobile, but let's try unmuted
